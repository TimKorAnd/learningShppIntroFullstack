<?php
    ...
    class Task12Controller
    {
        ...
        private function makeCalculation(int $from, int $to, array &$results): void
        {
            ...
            //global $results;
            try {
                $from = intval($_POST['task1-2']['from']);
                $to = intval($_POST['task1-2']['to']);
                foreach ($_POST['task1-2']['taskStatus'] as $taskStatus => $v) {
                    /*dumper($taskStatus);
                    echo $v;*/
                    if ($v === '0') {
        ...
        public function getResultsTask12()
        {
            ...
            /*$isTask1 = !empty($_POST['task1-2']['taskStatus']['task1']);
            $isTask2 = !empty($_POST['task1-2']['taskStatus']['task2']);*/
            if (empty($_POST['task1-2']['taskStatus']['task1']) &&
              empty($_POST['task1-2']['taskStatus']['task2'])) {
